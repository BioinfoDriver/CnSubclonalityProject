...
clinical.data$case_overall_survival_mo[clinical.data$case_overall_survival_mo == 'NULL'] <- NA
clinical.data$case_overall_survival_mo <- as.numeric(clinical.data$case_overall_survival_mo)

clinical.outcomes <- read.csv(file='clinical_outcomes.tsv', header=TRUE, sep='\t', stringsAsFactors=FALSE)
clinical.data <- merge(clinical.data, clinical.outcomes, by='case_barcode', all.x = TRUE)

clinical.data$case_vital_status[clinical.data$case_vital_status == 'NULL'] <- NA
clinical.data$case_vital_status <- ifelse(clinical.data$case_vital_status=='dead', 1,  0)
clinical.data$vital_status <- ifelse(clinical.data$vital_status == 'deceased', 1,  0)


clinical.data <- clinical.data[, c('case_barcode', 'case_sex', 'case_age_diagnosis_years', 'case_overall_survival_mo', 
 'case_vital_status', 'Time_to_first_recurrence', 'ttt_status', 'Post_recurrence_survival', 'prs_status')]
colnames(clinical.data) <- c('case_barcode', 'gender', 'age', 'os_time', 'os', 'ttt_time', 'ttt', 'prs_time', 'prs')

# saveRDS(clinical.data, file='/data/glass_sur_data.rds')
# load data
...
SubcloGenomeSurvPlot<- function(dat, subtype, feature, method, file.name){
 ...
 surv.plots <- lapply(c('os', 'ttt', 'prs'), function(x){
  
  dt <- dat[, c('case_barcode', x, paste0(x, '_time'), 'sample.label')] 
  colnames(dt) <- c('case_barcode', 'event', 'time', 'sample.label')
  dt <- subset(dt, !(is.na(event) | is.na(time)))
  
  
   sur.type <- switch(x,
				   os = 'Overall survival',
				   ttt = 'Time to first recurrence',
				   prs = 'Post recurrence survival')
				   
  sur.plot <-try(plot.surv(dt, xscale=1, ylab=sur.type, xlab="Time (months)")) # surv.median.line="hv", 
...
glass.cna.ith <- merge(glass.cna.ith, clinical.data, by='case_barcode')
glass.cna.ith$cna.bur <- glass.cna.ith$subclonal.cna.bur + glass.cna.ith$clonal.cna.bur 
glass.cna.ith$cna.bur.t <- glass.cna.ith$subclonal.cna.bur.t + glass.cna.ith$clonal.cna.bur.t 
glass.cna.ith$cna.bur.r <- glass.cna.ith$subclonal.cna.bur.r + glass.cna.ith$clonal.cna.bur.r 


cutoff.method <- c('median')
setwd('/result/Validation/scna_ith_survival')
SubcloGenomeSurvPlot(glass.cna.ith, 'Glioblastoma,IDHwt', 'cna.bur.t', cutoff.method, 't_IDHwt_genome_sur.pdf')
SubcloGenomeSurvPlot(glass.cna.ith, 'Astrocytoma,IDHmut', 'cna.bur.t', cutoff.method, 't_IDHmutNonCodel_genome_sur.pdf')

SubcloGenomeSurvPlot(glass.cna.ith, 'Glioblastoma,IDHwt', 'subclonal.cna.bur.t', cutoff.method, 't_IDHwt_subclo_genome_sur.pdf')
SubcloGenomeSurvPlot(glass.cna.ith, 'Astrocytoma,IDHmut', 'subclonal.cna.bur.t', cutoff.method, 't_IDHmutNonCodel_subclo_genome_sur.pdf')

SubcloGenomeSurvPlot(glass.cna.ith, 'Glioblastoma,IDHwt', 'clonal.cna.bur.t', cutoff.method, 't_IDHwt_clo_genome_sur.pdf')
SubcloGenomeSurvPlot(glass.cna.ith, 'Astrocytoma,IDHmut', 'clonal.cna.bur.t', cutoff.method, 't_IDHmutNonCodel_clo_genome_sur.pdf')
